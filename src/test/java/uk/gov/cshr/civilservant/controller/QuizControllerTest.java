...
public class QuizControllerTest {
    ...
    public void shouldCreateQuizForProfession() throws Exception {
        ...
        quiz.setId(1L);
        when(quizService.create(anyLong(), anyLong())).thenReturn(quiz);

        //then

        mockMvc.perform(
                post("/api/quiz").with(csrf())
                        .content(quizJson)
    ...
    public void shouldUpdateQuiz() throws Exception {
        ...
        mockMvc.perform(
                post("/api/quiz/update").with(csrf())
                        .content(quizJson)
    ...
    public void shouldGetHistoryForStaff() throws Exception {
        ...
        mockMvc.perform(
                get("/api/quiz/quiz-history?staffId=1232").with(csrf())
                        .accept(MediaType.APPLICATION_JSON))
                .andDo(print())
    ...
    public void shouldGetQuizSummary() throws Exception {
        ...
        mockMvc.perform(
                get("/api/quiz/quiz-summary").with(csrf())
    ...
    public void shouldSubmitAnswersToQuiz() throws Exception {
        ...
        mockMvc.perform(
                post("/api/quiz/submit-answers").with(csrf())
    ...
    public void shouldPublishQuiz() throws Exception{
        ...
        mockMvc.perform(
                put("/api/quiz/publish").with(csrf())
                        .content(quizJson)
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk())
