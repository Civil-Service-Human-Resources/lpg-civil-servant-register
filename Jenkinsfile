            ...
            steps {
                ...
                script {
                    docker.withRegistry("${env.DOCKER_REGISTRY_URL}", 'docker_registry_credentials') {
                        def customImage = docker.build("civil-servant-registry-service")
            ...
            steps {
                ...
                withCredentials([
                    ...
                    dir("lpg-terraform-paas/environments/master") {
                        ...
                        sh "terraform plan -target=module.civil-servant-registry-service -var 'civil_servant_registry_docker_tag=${env.BRANCH_NAME}-${env.BUILD_ID}' -var 'docker_registry_server_username=${acr_username}' -var 'docker_registry_server_password=${acr_password}'"
                        sh "terraform apply -target=module.civil-servant-registry-service -var 'civil_servant_registry_docker_tag=${env.BRANCH_NAME}-${env.BUILD_ID}' -var 'docker_registry_server_username=${acr_username}' -var 'docker_registry_server_password=${acr_password}' -auto-approve"
                    }
                }
            }
        }
